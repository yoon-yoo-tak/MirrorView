  ...
  const handleModal = () => {
    if (!user) {
      Swal.fire({
        ...
        title:
          '<div style="font-size:20px; font-family: HakgyoansimWoojuR;font-weight:bold;">로그인 후 이용 가능합니다.<div>',
        icon: "error",
        width: 330,
          ...
          <S.studylistContainer>
            <S.studylistTop>
              <S.testWrap>
                ...
                <S.divText>
                  {room.length}개의 스터디방이 기다리고 있어요!
                </S.divText>
              </S.testWrap>
              <StudyRoomCategory />
            </S.studylistTop>
            <S.studylistMain>
              {room.slice(offset, offset + 9).map((props, index) => (
