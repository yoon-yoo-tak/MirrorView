...
const StudyMyEssay = () => {
  ...
  const initialSelectedValueIndex =
    userInfo.essays && userInfo.essays.length > 0 ? 0 : -1;
  ...
  return (
    <div>
      <S.myEssayWrap>
        <S.myEssaySide>
          <S.myEssayIntro>{userInfo.nickname}님의 자기소개서</S.myEssayIntro>
          {!isMainChecked && (
            <S.myEssayMain>
          ...
          {mainEssay === selectedValueIndex && (
            <S.myEssayMain>메인 자소서로 설정되어있어요</S.myEssayMain>
          )}
        </S.myEssaySide>
        <hr />
