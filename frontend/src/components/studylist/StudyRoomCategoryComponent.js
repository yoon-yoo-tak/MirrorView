...
const StudyRoomCategory = () => {
  ...
  const handleSubmit = () => {
    ...
    if (thirdValue != "선택하세요") {
      dispatch(getInterviewRoomByCategory({ depth: 3, category: thirdValue }));
    } else if (secondValue != "선택하세요") {
      dispatch(getInterviewRoomByCategory({ depth: 2, category: secondValue }));
    } else if (firstValue == "선택하세요") {
      alert("카테고리를 선택하세요");
    } else {
      // console.log("good start");
      dispatch(getInterviewRoomByCategory({ depth: 1, category: firstValue }));
    }
  };
